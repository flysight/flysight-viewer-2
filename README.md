...
- **Debug builds.** The exported targets only carry the configurations that were built. A Debug application build needs a Debug build of both libraries as well (`cmake --build build --config Debug`); against a Release-only install it would mix MSVC runtimes. The configure warns about this when the configuration is chosen with `CMAKE_BUILD_TYPE`; with a multi-config generator (Visual Studio) the configuration is chosen at build time and nothing can warn.
