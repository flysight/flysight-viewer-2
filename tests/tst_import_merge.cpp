// Import and merge rules (new session versus merge, attribute conflicts, the
// measurement merge, the effects of a merge) through the application's import
...
// ─────────────────────────────── creation (no session has the file's SESSION_ID)
...
// ─────────────────────────────── attribute conflicts (acceptance 7)
...
// ─────────────────────────────── what survives a merge (acceptance 7)
...
void ImportMergeTest::unmatchedMeasurementsSurvive_data()
{
    QTest::addColumn<bool>("unloaded");
    QTest::newRow("loaded") << false;
    QTest::newRow("stub") << true;      // the session is an unloaded stub when the file arrives
}
...
// The attribute merge rule for Viewer's own (`_`) attributes, through the model: a
...
// ─────────────────────────────── existing sessions that cannot be loaded
