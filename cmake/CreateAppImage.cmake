...
if(FLYSIGHT_APPIMAGETOOL_AVAILABLE)
    ...
    set(APPIMAGE_SCRIPT_TEMPLATE [=[#!/bin/bash
...
if [ -f "${OUTPUT_PATH}" ]; then
    ...
    # Configure the script template with actual paths.
    # NOTE: string(CONFIGURE) must receive the template CONTENT. Passing the
    # script PATH here once generated a script containing only its own path,
    # which executed itself in an infinite bash recursion that took down CI
    # runners via memory exhaustion.
    string(CONFIGURE "${APPIMAGE_SCRIPT_TEMPLATE}" APPIMAGE_SCRIPT_CONTENT @ONLY)
    file(GENERATE OUTPUT "${APPIMAGE_SCRIPT}"
